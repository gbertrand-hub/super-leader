...
import { sendAccessRecoveryEmail } from "@/lib/auth/access-recovery-email";
...
async function getContext() {
  ...
  await enforceOrganizationFeature(membership.organization_id, "teams");
...
async function requireTeamMemberAssignmentAccess() {
  const context = await getContext();
  if (!canManageTeamMembers(context.membership.role)) {
...
export async function assignMemberToTeamAction(formData: FormData) {
  const { user, membership, t } = await requireTeamMemberAssignmentAccess();
  ...
  if (membership.role === "manager") {
    ...
    if (!teamAllowed || !userAllowed) {
      go(t("members.actionMessages.managerAssignmentOutsideScope"), "error");
    }
  }
...
export type MemberRecoveryAccessState = {
  status: "idle" | "success" | "warning" | "error";
  message?: string;
  setupLink?: string;
  emailSent?: boolean;
...
async function loadResettableMember(input: {
  admin: ReturnType<typeof createAdminClient>;
  memberId: string;
  organizationId: string;
  actorUserId: string;
  t: Translator;
}): Promise<ResettableMember> {
  const { data: target, error: targetError } = await input.admin
    .from("organization_members")
  ...
  if (targetError) {
    throw new Error(
      input.t("members.actionMessages.loadImpossible", {
  ...
  if (!target) {
    throw new Error(input.t("members.actionMessages.memberNotFound"));
  }

  if (target.role === "owner") {
    throw new Error(input.t("members.actionMessages.ownerAccessLocked"));
  }

  if (target.user_id === input.actorUserId) {
    throw new Error(input.t("members.actionMessages.cannotResetSelf"));
  }

  if (!target.is_active) {
    throw new Error(input.t("members.actionMessages.inactiveMemberAccess"));
  }

  const { data: authData, error: authError } = await runAuthAdminWithRetry(
    () => input.admin.auth.admin.getUserById(target.user_id),
    input.t("members.actionMessages.emptyAuthResult"),
  );

  if (authError || !authData.user) {
    throw new Error(
      input.t("members.actionMessages.accountLoadImpossible", {
        message: authError
          ? improveAuthAdminError(authError.message)
          : input.t("members.actionMessages.noReturnedUser"),
      }),
    );
  }
  ...
  const email = String(authData.user.email ?? profile?.email ?? "")
    .trim()
    .toLowerCase();
  if (!email) {
    throw new Error(input.t("members.actionMessages.emailMissingForReset"));
  }

  const fullName =
    String(profile?.full_name ?? authData.user.user_metadata?.full_name ?? "").trim() ||
    fallbackFullName(email);
...
export async function sendMemberRecoveryLinkAction(
  _previousState: MemberRecoveryAccessState,
  formData: FormData,
): Promise<MemberRecoveryAccessState> {
  const { user, membership, t } = await requirePeopleAdmin();
  ...
  if (!memberId) {
    return {
      status: "error",
      message: t("members.actionMessages.invalidMember"),
    };
  }
  ...
  try {
    const target = await loadResettableMember({
      admin,
      memberId,
      organizationId: membership.organization_id,
      actorUserId: user.id,
    ...
    const setupLink = await generatePasswordSetupLink(admin, target.email, t);
    const { data: organization } = await admin
      .from("organizations")
      .select("name")
    ...
    if (shouldSendEmail) {
      const emailResult = await sendAccessRecoveryEmail({
        to: target.email,
        fullName: target.fullName,
        recoveryLink: setupLink,
        locale,
        organizationName: organization?.name ?? "Super Leader",
      });
      emailSent = emailResult.sent;
    ...
    if (shouldSendEmail && !emailSent) {
      return {
        status: "warning",
        message: t("members.actionMessages.recoveryLinkEmailFailed", {
          email: target.email,
          message: emailWarning || t("common.unknownError"),
        }),
        setupLink,
    ...
    return {
      ...
      message: shouldSendEmail
        ? t("members.actionMessages.recoveryLinkSent", {
            email: target.email,
          })
        : t("members.actionMessages.recoveryLinkGenerated", {
            email: target.email,
          }),
      setupLink,
      emailSent,
    ...
    return {
      ...
      message:
        error instanceof Error
          ? error.message
          : t("members.actionMessages.unknownAccessResetError"),
    };
  }
}

export async function resetMemberWithTemporaryPasswordAction(
  _previousState: TemporaryPasswordAccessState,
  formData: FormData,
): Promise<TemporaryPasswordAccessState> {
  const { user, membership, t } = await requirePeopleAdmin();
  ...
  try {
    ...
    const { data: authData, error: authError } = await runAuthAdminWithRetry(
      () => admin.auth.admin.getUserById(target.userId),
      t("members.actionMessages.emptyAuthResult"),
    );

    if (authError || !authData.user) {
      throw new Error(
        t("members.actionMessages.accountLoadImpossible", {
          message: authError
            ? improveAuthAdminError(authError.message)
            : t("members.actionMessages.noReturnedUser"),
        }),
    ...
    const { data: updateData, error: updateError } = await runAuthAdminWithRetry(
      () =>
        admin.auth.admin.updateUserById(target.userId, {
          password: temporaryPassword,
          email_confirm: true,
          user_metadata: {
            ...currentMetadata,
    ...
    await admin.from("temporary_access_audit_log").insert({
      ...
      actor_user_id: user.id,
      metadata: {
        method: "member_card_temporary_password",
        expires_at: expiresAt,
        email_requested: shouldSendEmail,
      },
    });
    ...
    return {
      ...
      message:
        shouldSendEmail && !emailSent
          ...
          : shouldSendEmail
            ? t("members.actionMessages.temporaryCreatedAndSent", {
                email: target.email,
              })
            : t("members.actionMessages.temporaryCreated", {
                email: target.email,
              }),
    };
  } catch (error) {
