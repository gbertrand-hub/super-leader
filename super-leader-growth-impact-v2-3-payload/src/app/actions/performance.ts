...
export async function calculateMonthlyPerformanceAction(formData: FormData) {
  ...
  const [settingsResult, membersResult, schedulesResult, detailedSchedulesResult, attendanceResult, leavesResult, reportsResult, meetingsResult, meetingAttendanceResult, feedbackResult, recognitionsResult, academyCoursesResult, academyEnrollmentsResult, kpiResult, growthSettingsResult, growthPlansResult, growthContributionsResult] = await Promise.all([
    ...
    admin.from("growth_settings").select("target_credits,bonus_weight,max_monthly_credits").eq("organization_id", membership.organization_id).maybeSingle(),
  ...
  const growthModuleError = growthSettingsResult.error && !["42P01", "PGRST205"].includes(growthSettingsResult.error.code) ? growthSettingsResult.error : null;
  ...
  const anyError = [settingsResult.error, membersResult.error, schedulesResult.error, detailedScheduleError, attendanceResult.error, leavesResult.error, reportsResult.error, meetingsResult.error, meetingAttendanceResult.error, feedbackResult.error, recognitionsResult.error, academyCoursesResult.error, academyEnrollmentsResult.error, kpiResult.error, growthModuleError, growthPlansError, growthContributionsError].find(Boolean);
  ...
  const growthSettings = (growthSettingsResult.data ?? {target_credits: 10, bonus_weight: 10, max_monthly_credits: 20}) as {target_credits: number | string; bonus_weight: number | string; max_monthly_credits: number | string};
  const growthPlanRows = (growthPlansResult.data ?? []) as {user_id: string; target_credits: number | string}[];
  const growthCreditsByUser = new Map<string, number>();
  for (const row of (growthContributionsResult.data ?? []) as {user_id: string; growth_credits: number | string}[]) {
    growthCreditsByUser.set(row.user_id, (growthCreditsByUser.get(row.user_id) ?? 0) + Number(row.growth_credits ?? 0));
  }
  ...
  const calculated = members.map((member) => ({
    ...
    result: calculatePerformanceScore({
      ...
      growth: {
        approvedCredits: growthCreditsByUser.get(member.user_id) ?? 0,
