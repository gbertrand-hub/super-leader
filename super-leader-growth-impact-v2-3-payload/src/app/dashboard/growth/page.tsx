...
export default async function GrowthPage({searchParams}: PageProps) {
  ...
  const [settingsResult, membersResult, profilesResult, planResult, contributionsResult] = await Promise.all([
    admin.from("growth_settings").select("default_monthly_target_hours,target_credits,bonus_weight,max_monthly_credits,night_start_time,night_end_time,wellbeing_warning_hours").eq("organization_id", membership.organization_id).maybeSingle<SettingsRow>(),
    ...
    admin.from("growth_plans").select("user_id,plan_month,target_hours,target_credits,focus_skill,objective,status").eq("organization_id", membership.organization_id).eq("user_id", selectedUserId).eq("plan_month", monthStart).maybeSingle<PlanRow>(),
  ...
  const migrationMissing = settingsResult.error?.code === "42P01" || settingsResult.error?.code === "PGRST205";
  if (migrationMissing) {
    return <main className="p-6 lg:p-10"><section className="mx-auto max-w-5xl rounded-3xl border border-amber-200 bg-amber-50 p-8"><p className="text-sm font-black uppercase tracking-[0.2em] text-amber-700">Super Leader V2.3</p><h1 className="mt-3 text-3xl font-black text-amber-950">{t("growth.setupTitle")}</h1><p className="mt-3 text-amber-900">{t("growth.setupHelp")}</p><code className="mt-5 block rounded-2xl bg-slate-950 p-4 text-sm text-white">supabase/027_growth_impact_v2_3.sql</code></section></main>;
  }

  const loadError = [settingsResult.error, membersResult.error, profilesResult.error, planResult.error, contributionsResult.error].find(Boolean);
  ...
  const wellbeingWarning = hours(nightMinutes + weekendMinutes) >= number(settings.wellbeing_warning_hours) && number(settings.wellbeing_warning_hours) > 0;
    ...
    <main className="p-5 sm:p-7 lg:p-10">
      <div className="mx-auto max-w-7xl space-y-7">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
          ...
          <Metric label={t("growth.metrics.credits")} value={credits.toFixed(1)} detail={t("growth.metrics.creditsHelp", {target: targetCredits})} tone="indigo" />
          <Metric label={t("growth.metrics.pending")} value={String(pending.length)} detail={t("growth.metrics.pendingHelp")} tone="amber" />
          <Metric label={t("growth.metrics.night")} value={`${hours(nightMinutes)} h`} detail={t("growth.metrics.nightHelp")} />
          <Metric label={t("growth.metrics.weekend")} value={`${hours(weekendMinutes)} h`} detail={t("growth.metrics.weekendHelp")} />
        </div>
        ...
        <section className="grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
          <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2"><div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black uppercase text-slate-500">{t("growth.hoursProgress")}</p><p className="mt-2 text-2xl font-black">{hourProgress}%</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-emerald-500" style={{width: `${hourProgress}%`}} /></div></div><div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black uppercase text-slate-500">{t("growth.creditProgress")}</p><p className="mt-2 text-2xl font-black">{creditProgress}%</p><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-indigo-600" style={{width: `${creditProgress}%`}} /></div></div></div>
            <form action={upsertGrowthPlanAction} className="mt-6 space-y-4"><input type="hidden" name="month" value={month} /><input type="hidden" name="userId" value={selectedUserId} /><div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-black">{t("growth.targetHours")}<input name="targetHours" type="number" min="0" max="200" step="0.5" defaultValue={targetHours} required className={fieldClass()} /></label><label className="text-sm font-black">{t("growth.targetCredits")}<input name="targetCredits" type="number" min="1" max="500" step="0.5" defaultValue={targetCredits} required className={fieldClass()} /></label></div><label className="block text-sm font-black">{t("growth.focusSkill")}<input name="focusSkill" defaultValue={plan?.focus_skill ?? ""} required className={fieldClass()} /></label><label className="block text-sm font-black">{t("growth.objective")}<textarea name="objective" rows={4} defaultValue={plan?.objective ?? ""} required className={fieldClass()} /></label><button className="w-full rounded-xl bg-indigo-700 px-5 py-3 font-black text-white">{t("growth.savePlan")}</button></form>
          </article>
        ...
        {canConfigure ? <section className="rounded-3xl border border-slate-300 bg-slate-50 p-6"><h2 className="text-2xl font-black">{t("growth.settingsTitle")}</h2><p className="mt-2 text-sm text-slate-600">{t("growth.settingsHelp")}</p><form action={updateGrowthSettingsAction} className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><input type="hidden" name="month" value={month} /><label className="text-sm font-black">{t("growth.defaultTargetHours")}<input name="defaultMonthlyTargetHours" type="number" min="0" max="200" step="0.5" defaultValue={number(settings.default_monthly_target_hours)} className={fieldClass()} /></label><label className="text-sm font-black">{t("growth.defaultTargetCredits")}<input name="targetCredits" type="number" min="1" max="500" step="0.5" defaultValue={number(settings.target_credits)} className={fieldClass()} /></label><label className="text-sm font-black">{t("growth.bonusWeight")}<input name="bonusWeight" type="number" min="0" max="20" step="0.5" defaultValue={number(settings.bonus_weight)} className={fieldClass()} /></label><label className="text-sm font-black">{t("growth.maxMonthlyCredits")}<input name="maxMonthlyCredits" type="number" min="1" max="1000" step="0.5" defaultValue={number(settings.max_monthly_credits)} className={fieldClass()} /></label><label className="text-sm font-black">{t("growth.nightStart")}<input name="nightStartTime" type="time" defaultValue={settings.night_start_time.slice(0,5)} className={fieldClass()} /></label><label className="text-sm font-black">{t("growth.nightEnd")}<input name="nightEndTime" type="time" defaultValue={settings.night_end_time.slice(0,5)} className={fieldClass()} /></label><label className="text-sm font-black">{t("growth.wellbeingWarningHours")}<input name="wellbeingWarningHours" type="number" min="0" max="200" step="0.5" defaultValue={number(settings.wellbeing_warning_hours)} className={fieldClass()} /></label><div className="flex items-end"><button className="w-full rounded-xl bg-slate-950 px-5 py-3 font-black text-white">{t("growth.saveSettings")}</button></div></form></section> : null}
      </div>
    </main>
  );
}
