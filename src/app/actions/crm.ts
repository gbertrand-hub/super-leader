...
async function getContext() {
  ...
  return {user: authData.user, membership, admin, t, locale};
}
...
function canAccessClient(role: string, userId: string, client: ClientRow) {
  return leaderRoles.has(role) || client.owner_id === userId || client.follow_up_owner_id === userId;
}
...
export async function createCrmClientAction(formData: FormData) {
  ...
  const followUpOwnerId = String(formData.get("followUpOwnerId") ?? "").trim() || null;
...
export async function updateCrmClientAction(formData: FormData) {
  ...
  if (!canAccessClient(membership.role, user.id, client)) go(returnTo, t("crm.messages.clientPermissionDenied"), "error");
...
export async function createCrmInteractionAction(formData: FormData) {
  ...
  if (!leaderRoles.has(membership.role) && employeeId !== user.id) go(returnTo, t("crm.messages.employeePermissionDenied"), "error");
...
export async function updateCrmTaskStatusAction(formData: FormData) {
  ...
  const {data: task} = await admin
    ...
    .eq("id", taskId)
    .eq("organization_id", membership.organization_id)
    .maybeSingle<{id: string; client_id: string; assigned_to: string | null}>();
  if (!task) go(returnTo, t("crm.messages.taskNotFound"), "error");
  if (!leaderRoles.has(membership.role) && task.assigned_to !== user.id) go(returnTo, t("crm.messages.taskPermissionDenied"), "error");
...
export async function resolveCustomerFeedbackAction(formData: FormData) {
  ...
  const {data: response} = await admin.from("crm_feedback_responses").select("id, client_id").eq("id", responseId).eq("organization_id", membership.organization_id).maybeSingle<{id: string; client_id: string}>();
