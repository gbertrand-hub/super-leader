...
type MeetingRow = {id: string; title: string; meeting_type: string; mandatory: boolean; starts_at: string; ends_at: string | null};
...
export default async function MyDayPage() {
  ...
  const [{data: settingsData}, {data: currentScheduleData}] = await Promise.all([
    admin.from("performance_settings").select("timezone, default_start_time, default_end_time, report_deadline_time, report_lock_enabled").eq("organization_id", membership.organization_id).maybeSingle<SettingsRow>(),
  ...
  const [attendanceResult, reportResult, reopeningResult, meetingsResult, tasksResult, collectionsResult, feedbackResult, notificationsResult, scoreResult, plansResult] = await Promise.all([
    admin.from("attendance_records").select("user_id, work_date, clock_in_at, clock_out_at, status, late_minutes").eq("organization_id", membership.organization_id).eq("user_id", authData.user.id).eq("work_date", today).maybeSingle<AttendanceRow>(),
    ...
    admin.from("performance_meetings").select("id, title, meeting_type, mandatory, starts_at, ends_at").eq("organization_id", membership.organization_id).gte("starts_at", meetingWindowStart.toISOString()).lte("starts_at", meetingWindowEnd.toISOString()).order("starts_at", {ascending: true}).limit(50),
  ...
  const optionalErrors = [reopeningResult.error, meetingsResult.error, tasksResult.error, collectionsResult.error, feedbackResult.error, notificationsResult.error, scoreResult.error, plansResult.error].filter(Boolean);
          ...
          <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              ...
              <form action={clockOutAction}>
                <button disabled={!attendance?.clock_in_at || Boolean(attendance.clock_out_at)} className="w-full rounded-xl border border-indigo-300 bg-indigo-50 px-5 py-3 font-black text-indigo-800 transition hover:bg-indigo-100 disabled:cursor-not-allowed disabled:opacity-40">
              ...
              {ownMeetings.length ? ownMeetings.map((meeting) => {
                ...
                return (
                  <div key={meeting.id} className="rounded-2xl border border-slate-200 p-4">
                    ...
                    <div className="mt-3"><StatusPill tone={statusTone(attendanceStatus)}>{t(`performance.meetingStatuses.${attendanceStatus}`)}</StatusPill></div>
                  </div>
                );
