...
export default async function EventsPage({searchParams}: PageProps) {
  ...
  const schedulePanel = selectedEvent ? <section id="schedule" className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
    ...
    {canManage ? <form id="add-schedule" action={createEventScheduleItemAction} className="mt-5 grid gap-3 rounded-2xl bg-indigo-50 p-4 lg:grid-cols-2"><input type="hidden" name="eventId" value={selectedEvent.id} /><label className="block text-sm font-black lg:col-span-2">{c.activityTitle}<input name="title" required className={fieldClass()} /></label><label className="block text-sm font-black">{c.type}<select name="itemType" defaultValue="session" className={fieldClass()}>{scheduleTypes.map((value) => <option key={value} value={value}>{labelValue(value, locale)}</option>)}</select></label><label className="block text-sm font-black">{c.status}<select name="status" defaultValue="planned" className={fieldClass()}><option value="planned">{labelValue("planned", locale)}</option><option value="confirmed">{labelValue("confirmed", locale)}</option><option value="completed">{labelValue("completed", locale)}</option><option value="cancelled">{labelValue("cancelled", locale)}</option></select></label><label className="block text-sm font-black">{c.start}<input name="startAt" type="datetime-local" required className={fieldClass()} /></label><label className="block text-sm font-black">{c.end}<input name="endAt" type="datetime-local" required className={fieldClass()} /></label><label className="block text-sm font-black">{c.owner}<select name="ownerId" defaultValue="" className={fieldClass()}><option value="">{c.unassigned}</option>{eventMembers.map((member) => <option key={member.user_id} value={member.user_id}>{eventProfileById.get(member.user_id)?.name ?? member.user_id}</option>)}</select></label><label className="block text-sm font-black">{c.unit}<input name="unitName" className={fieldClass()} /></label><label className="block text-sm font-black">{c.location}<input name="location" className={fieldClass()} /></label><label className="block text-sm font-black">{c.meetingUrl}<input name="meetingUrl" type="url" className={fieldClass()} /></label><div className="space-y-3 lg:col-span-2"><label className={`flex items-center gap-3 rounded-xl p-4 text-sm font-bold ${zoomEnabled ? "bg-blue-100 text-blue-950" : "bg-amber-100 text-amber-950"}`}><input name="createZoom" type="checkbox" disabled={!zoomEnabled} />{zoomEnabled ? (locale === "fr" ? "Créer automatiquement la réunion Zoom (type Réunion uniquement)" : "Automatically create the Zoom meeting (Meeting type only)") : (locale === "fr" ? "Configure et synchronise les comptes Zoom dans Intégrations" : "Configure and sync Zoom accounts in Integrations")}</label>{zoomEnabled ? <ZoomHostSelector hosts={zoomHostOptions} departments={zoomDepartments} defaultHostId={defaultZoomHostId} locale={locale} /> : null}</div><label className="block text-sm font-black lg:col-span-2">{c.notes}<textarea name="notes" className={textAreaClass()} /></label><button className="rounded-xl bg-indigo-600 px-5 py-3 font-black text-white lg:col-span-2">{c.addSchedule}</button></form> : null}
