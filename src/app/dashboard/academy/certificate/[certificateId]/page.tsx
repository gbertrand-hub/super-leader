...
export default async function AcademyCertificatePage({params}: Props) {
  ...
  const {data: certificate} = await admin.from("academy_certificates").select("id, organization_id, course_id, enrollment_id, user_id, certificate_number, verification_token, final_score, issued_at, status").eq("id", certificateId).eq("organization_id", membership.organization_id).maybeSingle<Certificate>();
        ...
        <article className="relative overflow-hidden rounded-[2.5rem] border-[10px] border-slate-950 bg-white px-10 py-14 shadow-2xl print:min-h-[190mm] print:rounded-none print:shadow-none">
          ...
          <div className="relative text-center">
            ...
            <div className="mx-auto mt-8 grid max-w-4xl gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black uppercase text-slate-500">{t("academy.certificate.attendance")}</p><p className="mt-1 font-black">{enrollment ? `${Number(enrollment.attendance_percent)} % (${enrollment.sessions_attended}/${enrollment.sessions_expected})` : "—"}</p></div>
            </div>
