...
import {ZoomHostSelector} from "@/components/zoom-host-selector";
...
export default async function PerformancePage({searchParams}: PageProps) {
  ...
  const [zoomSettings, zoomHosts, zoomTeamsResult] = await Promise.all([
    getOrganizationZoomSettings(admin, membership.organization_id),
    getOrganizationZoomHosts(admin, membership.organization_id, {activeOnly: true}),
    admin.from("teams").select("department").eq("organization_id", membership.organization_id).eq("is_active", true),
  ]);
  if (zoomTeamsResult.error && !["42P01", "PGRST205"].includes(zoomTeamsResult.error.code)) throw new Error(zoomTeamsResult.error.message);
  const zoomEnabled = Boolean(zoomSettings.enabled && (zoomHosts.length || zoomSettings.default_host_email));
  const zoomDepartments = [...new Set([
    ...(zoomTeamsResult.data || []).map((team) => String(team.department || "").trim()),
    ...zoomHosts.map((host) => String(host.department || "").trim()),
  ].filter(Boolean))].sort((a, b) => a.localeCompare(b));
  const zoomHostOptions = zoomHosts.map((host) => ({
    id: host.id,
    email: host.email,
    displayName: host.display_name || host.email,
    department: host.department || "",
    isDepartmentDefault: host.is_department_default,
  }));
  const defaultZoomHostId = zoomHosts.find((host) => host.email.toLocaleLowerCase() === String(zoomSettings.default_host_email || "").toLocaleLowerCase())?.id || zoomHosts[0]?.id || null;
  ...
  const [schedulesResult, detailedSchedulesResult, attendanceResult, attendanceReopeningsResult, leavesResult, reportsResult, reportVersionsResult, reopeningsResult, meetingsResult, meetingAttendanceResult, kpiResult, scoresResult, awardsResult, appealsResult] = await Promise.all([
    ...
    admin.from("performance_meetings").select("id, title, meeting_type, mandatory, starts_at, ends_at, notes, created_by, provider, meeting_url, zoom_meeting_id, zoom_host_email, zoom_department, zoom_status, zoom_last_synced_at, zoom_sync_error").eq("organization_id", membership.organization_id).gte("starts_at", `${monthStart}T00:00:00Z`).lte("starts_at", `${monthEnd}T23:59:59Z`).order("starts_at", {ascending: false}).limit(200),
