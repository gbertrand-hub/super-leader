...
export default async function AcademyPage({searchParams}: Props) {
  ...
  const [{data: organization}, profilesResult, coursesResult, enrollmentsResult, certificatesResult] = await Promise.all([
    ...
    admin.from("academy_courses").select("id, title, description, category, growth_program_code, training_month, deadline, duration_minutes, is_required, passing_score, max_attempts, certificate_enabled, attendance_required_percent, resource_url, status, created_at").eq("organization_id", membership.organization_id).order("training_month", {ascending: false}).order("created_at", {ascending: false}),
                  ...
                  <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <form action={updateAcademyCourseAction} className="mt-5 space-y-4">
                      ...
                      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        <label className="block text-sm font-black">{t("growth.development.program")}<select name="growthProgramCode" defaultValue={selectedCourse.growth_program_code || "other_training"} className={fieldClass()}>{["school_coaches","school_business","school_experts","school_breeders","vision_monday","other_training"].map((value) => <option key={value} value={value}>{t(`growth.development.programs.${value}`)}</option>)}</select></label>
