...
export function AcademyCourseWizard({locale, currentMonth, participants, teams}: Props) {
  ...
  const c = fr
    ? {
        ...
        category: "Catégorie",
        growthProgram: "Parcours de croissance",
        programOptions: {
          school_coaches: "École des Coachs",
          school_business: "École des Affaires",
          school_experts: "École des Experts",
          school_breeders: "École des Éleveurs",
          vision_monday: "Lundi de la Vision",
    ...
    : {
        ...
        deadline: "Deadline",
        category: "Category",
        growthProgram: "Growth pathway",
        programOptions: {
          school_coaches: "School of Coaches",
          school_business: "School of Business",
          school_experts: "School of Experts",
          school_breeders: "School of Breeders",
          vision_monday: "Vision Monday",
  ...
  return (
    <article className="rounded-3xl border border-indigo-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-indigo-600">V2.3.1</p>
      ...
      <form ref={formRef} action={createAcademyCourseWizardAction} noValidate className="mt-6">
        <section className={step === "details" ? "space-y-5" : "hidden"}>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <label className="block text-sm font-black">{c.growthProgram}<select name="growthProgramCode" defaultValue="other_training" className={inputClass()}>{Object.entries(c.programOptions).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
