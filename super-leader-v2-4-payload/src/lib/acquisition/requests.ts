...
export async function saveDemoRequest(input: DemoRequestInput): Promise<string> {
  ...
  await notifyPlatformReviewers({
    ...
    bodyFr: `${input.fullName} souhaite découvrir Super Leader pour ${input.organizationName}.`,
    bodyEn: `${input.fullName} would like to discover Super Leader for ${input.organizationName}.`,
