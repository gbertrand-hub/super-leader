...
export default async function AcademyPage({searchParams}: Props) {
  ...
  const schemaCheck = recurringSchemaCheck;
      ...
      <main className="min-h-screen bg-slate-50 px-5 py-8 text-slate-950">
        <div className="mx-auto max-w-4xl">
          ...
          <section className="mt-6 rounded-3xl border border-amber-200 bg-amber-50 p-7">
            ...
            {missingSchema ? <code className="mt-5 block rounded-xl bg-slate-950 px-4 py-3 font-bold text-white">{academySchemaCheck.error ? "supabase/021_super_leader_academy_v1.sql" : "supabase/022_academy_recurring_sessions_v2_2_1.sql"}</code> : null}
          </section>
        </div>
      </main>
    );
  ...
  const [schedulesResult, sessionsResult, attendanceResult] = selectedCourse
    ? await Promise.all([
        admin.from("academy_course_schedules").select("id, course_id, label, schedule_type, starts_on, ends_on, local_start_time, duration_minutes, timezone, weekdays, monthly_start_day, consecutive_days, zoom_join_url, is_active").eq("organization_id", membership.organization_id).eq("course_id", selectedCourse.id).order("starts_on"),
              ...
              <>
                ...
                {isAcademyAdmin ? (
                  <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    <div className="mt-5 space-y-3">
                      {selectedSchedules.map((schedule) => <div key={schedule.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-black">{schedule.label}</p><p className="mt-1 text-sm text-slate-600">{scheduleSummary(schedule, locale)} · {formatSessionTime(schedule.local_start_time)} · {schedule.duration_minutes} min</p><p className="mt-1 text-xs text-slate-500">{formatDate(schedule.starts_on, locale)} → {formatDate(schedule.ends_on, locale)} · {schedule.timezone}</p></div><span className="rounded-full bg-violet-100 px-3 py-1 text-xs font-black text-violet-800">{t(`academy.recurring.${schedule.schedule_type === "monthly_intensive" ? "monthlyIntensive" : schedule.schedule_type}`)}</span></div></div>)}
                ...
                <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                  ...
                  <div className="mt-5 space-y-4">
                    ...
                    {!visibleSessions.length ? <p className="rounded-2xl bg-slate-50 p-8 text-center text-slate-500">{t("academy.sessions.none")}</p> : null}
                  </div>
                </article>
                ...
                <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                    ...
                    {!ownEnrollment ? (
                      ...
                      <div className="mt-5">
                        ...
                        {["in_progress", "failed"].includes(ownEnrollment.status) && Number(ownEnrollment.best_score ?? 0) < Number(selectedCourse.passing_score) && ownEnrollment.attempts_count < selectedCourse.max_attempts ? (
                        ...
                        {Number(ownEnrollment.best_score ?? 0) >= Number(selectedCourse.passing_score) && ownEnrollment.status !== "completed" ? <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-900"><p className="text-xl font-black">{t("academy.attendancePendingTitle")}</p><p className="mt-1">{t("academy.attendancePendingHelp", {attendance: ownEnrollment.attendance_percent, required: selectedCourse.attendance_required_percent})}</p></div> : null}
