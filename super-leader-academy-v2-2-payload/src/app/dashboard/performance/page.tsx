...
export default async function PerformancePage({searchParams}: PageProps) {
  ...
  const [schedulesResult, detailedSchedulesResult, attendanceResult, leavesResult, reportsResult, reopeningsResult, meetingsResult, meetingAttendanceResult, kpiResult, scoresResult, awardsResult, appealsResult] = await Promise.all([
    ...
    admin.from("attendance_records").select("id, user_id, work_date, clock_in_at, clock_out_at, status, late_minutes, justification, source").eq("organization_id", membership.organization_id).in("user_id", visibleUserIds).gte("work_date", monthStart).lte("work_date", monthEnd).order("work_date", {ascending: false}).limit(500),
    ...
    admin.from("daily_reports").select("id, user_id, report_date, accomplishments, results, blockers, next_priorities, status, review_note, submitted_at, submitted_by, submission_mode, submission_score_factor, supervisor_reason, reopening_id").eq("organization_id", membership.organization_id).in("user_id", visibleUserIds).gte("report_date", monthStart).lte("report_date", monthEnd).order("report_date", {ascending: false}).limit(500),
    ...
    admin.from("performance_meetings").select("id, title, meeting_type, mandatory, starts_at, ends_at, notes, created_by").eq("organization_id", membership.organization_id).gte("starts_at", `${monthStart}T00:00:00Z`).lte("starts_at", `${monthEnd}T23:59:59Z`).order("starts_at", {ascending: false}).limit(200),
    admin.from("performance_meeting_attendance").select("id, meeting_id, user_id, status, late_minutes, notes").eq("organization_id", membership.organization_id).in("user_id", visibleUserIds),
    ...
    admin.from("employee_month_scores").select("user_id, score_month, attendance_score, punctuality_score, meetings_score, reports_score, collaboration_score, training_score, role_kpi_score, total_score, scheduled_days, attended_days, late_days, unexcused_absences, reports_expected, reports_submitted, mandatory_meetings, meetings_attended, trainings_required, trainings_completed, eligible, eligibility_note, position, calculated_at, locked_at").eq("organization_id", membership.organization_id).in("user_id", visibleUserIds).eq("score_month", scoreMonth).order("total_score", {ascending: false}),
