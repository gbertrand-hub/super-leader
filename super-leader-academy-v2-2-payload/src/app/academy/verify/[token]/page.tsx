...
export default async function VerifyAcademyCertificatePage({params}: Props) {
  ...
  const {data: certificate} = await admin.from("academy_certificates").select("id, organization_id, course_id, user_id, certificate_number, final_score, issued_at, status").eq("verification_token", token).maybeSingle<{
    organization_id: string; course_id: string; user_id: string; certificate_number: string; final_score: number | string | null; issued_at: string; status: string;
  }>();
