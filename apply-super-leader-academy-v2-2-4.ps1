...
$payloadRoot = Join-Path $scriptRoot "super-leader-academy-v2-2-4-payload"
...
if (-not (Test-Path $payloadRoot)) {
  Write-Host "ERROR: The folder super-leader-academy-v2-2-4-payload is missing." -ForegroundColor Red
  Read-Host "Press Enter to close"
  exit 1
}

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$backupRoot = Join-Path $projectRoot "backup-academy-v2-2-4-$timestamp"
$files = @(
...
Write-Host "Applying Super Leader Academy V2.2.4..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "SUCCESS: Super Leader Academy V2.2.4 has been installed." -ForegroundColor Green
Write-Host "REQUIRED: Run supabase/025_academy_course_creation_wizard_v2_2_4.sql in Supabase SQL Editor." -ForegroundColor Yellow
