...
import { isPeopleAdmin, isTeamManager } from "@/lib/auth/permissions";
...
type Team = { id: string; name: string; department: string | null; created_by?: string | null };
type Assignment = {
  team_id: string;
  user_id: string;
...
export default async function MembersPage({
  ...
  const canManageTeams = isTeamManager(currentMembership.role);
  ...
  const [membersResult, teamsResult, assignmentsResult, invitationsResult] =
    await Promise.all([
      ...
      admin
        .from("teams")
        .select("id,name,department,created_by")
        .eq("organization_id", organizationId)
        .order("name"),
      admin
        .from("team_members")
  ...
  const scopedTeamIds = new Set(assignments.map((assignment) => assignment.team_id));
  const teams = currentMembership.role === "manager"
    ? allTeams.filter(
        (team) => team.created_by === authData.user.id || scopedTeamIds.has(team.id),
      )
    : allTeams;
