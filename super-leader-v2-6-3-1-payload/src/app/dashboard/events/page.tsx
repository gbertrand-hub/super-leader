          ...
          {!selectedEvent ? <div className="rounded-3xl border border-slate-200 bg-white p-10 text-center text-slate-500">{c.selectEvent}</div> : <>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                <h3 className="text-2xl font-black text-slate-950">{c.tasks}</h3>
                <Badge tone="amber">{taskDone}/{tasks.length}</Badge>
              </div>
                ...
                {tasks.length ? tasks.map((task) => {
                  ...
                  return <article key={task.id} className="rounded-2xl border border-slate-200 p-5">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <Badge tone={statusTone(task.priority)}>{labelValue(task.priority, locale)}</Badge>
                          <Badge tone={statusTone(task.status)}>{labelValue(task.status, locale)}</Badge>
                        </div>
                        <p className="mt-2 text-sm text-slate-500">{task.milestone || "—"} · {task.assignee_id ? eventProfileById.get(task.assignee_id)?.name ?? task.assignee_id : c.unassigned}</p>
                        {task.due_at ? <p className="mt-1 text-xs font-bold text-slate-500">{c.due}: {formatDateTime(task.due_at, selectedEvent.timezone, dateLocale)}</p> : null}
                      </div>
                      <p className="text-2xl font-black text-indigo-700">{task.progress}%</p>
                    </div>
                    ...
                    {canUpdateTask ? <form action={updateEventTaskAction} className="mt-4 rounded-xl bg-slate-50 p-4">
                      ...
                      {canManage ? <details open={!task.assignee_id} className="rounded-xl border border-indigo-200 bg-white p-4">
                        <summary className="cursor-pointer font-black text-indigo-800">{c.editTask}</summary>
                        <p className="mt-2 text-xs leading-5 text-slate-500">{c.editTaskHelp}</p>
                        <div className="mt-4 grid gap-3 lg:grid-cols-2">
                          <label className="block text-xs font-black lg:col-span-2">{c.taskTitle}<input name="title" required defaultValue={task.title} className={fieldClass()} /></label>
                          <label className="block text-xs font-black">{c.milestone}<input name="milestone" defaultValue={task.milestone ?? ""} className={fieldClass()} /></label>
                          <label className="block text-xs font-black">{c.assignee}<select name="assigneeId" defaultValue={task.assignee_id ?? ""} className={fieldClass()}><option value="">{c.unassigned}</option>{assignableEventMembers.map((member) => <option key={member.user_id} value={member.user_id}>{eventProfileById.get(member.user_id)?.name ?? member.user_id} · {member.mission_role}</option>)}</select></label>
                          <label className="block text-xs font-black">{c.priority}<select name="priority" defaultValue={task.priority} className={fieldClass()}>{priorities.map((value) => <option key={value} value={value}>{labelValue(value, locale)}</option>)}</select></label>
                          <label className="block text-xs font-black">{c.due}<input name="dueAt" type="datetime-local" defaultValue={localDateTime(task.due_at, selectedEvent.timezone)} className={fieldClass()} /></label>
                          <label className="block text-xs font-black">{c.budget}<input name="budgetEstimate" type="number" min="0" step="0.01" defaultValue={task.budget_estimate ?? ""} className={fieldClass()} /></label>
                          <label className="block text-xs font-black">{c.currency}<input name="currency" defaultValue={task.currency || selectedEvent.currency} maxLength={3} className={fieldClass()} /></label>
                          <label className="block text-xs font-black lg:col-span-2">{c.description}<textarea name="description" defaultValue={task.description ?? ""} className={textAreaClass()} /></label>
                        </div>
                      </details> : null}

                      <div className="mt-4 grid gap-3 lg:grid-cols-4">
                        <label className="block text-xs font-black">{c.status}<select name="status" defaultValue={task.status} className={fieldClass()}>{taskStatusValues.map((value) => <option key={value} value={value}>{labelValue(value, locale)}</option>)}</select></label>
                        <label className="block text-xs font-black">{c.progress}<input name="progress" type="number" min="0" max="100" defaultValue={task.progress} className={fieldClass()} /></label>
                        <label className="block text-xs font-black">{c.proof}<input name="proofUrl" type="url" defaultValue={task.proof_url ?? ""} className={fieldClass()} /></label>
                        <label className="block text-xs font-black">{c.actualCost}<input name="actualCost" type="number" min="0" step="0.01" defaultValue={task.actual_cost ?? ""} disabled={!canManage} className={fieldClass()} /></label>
                        <label className="block text-xs font-black lg:col-span-3">{c.notes}<input name="notes" defaultValue={task.notes ?? ""} className={fieldClass()} /></label>
                        <button className="self-end rounded-xl bg-indigo-600 px-4 py-3 text-sm font-black text-white">{canManage ? c.saveTask : c.update}</button>
                      </div>
                    </form> : null}
                  </article>;
