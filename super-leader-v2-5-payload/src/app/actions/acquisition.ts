...
export async function convertDemoRequestAction(formData: FormData) {
  ...
  const { data: request, error: requestError } = await admin
    .from("demo_requests")
    .select("id,requester_user_id,email,organization_name,sector,status,converted_organization_id")
    .eq("id", requestId)
    .maybeSingle<{
      id: string;
  ...
  if (!["demo_completed", "trial_approved"].includes(request.status)) {
    go("Termine d’abord la démonstration ou approuve l’essai avant la conversion.", "error", "demo");
  }
  ...
  // V2.5: attribue automatiquement un essai Starter lorsque le module abonnement est actif.
  try {
    const { data: starterPlan } = await admin
      .from("subscription_plans")
      .select("id,currency,default_trial_days")
      .eq("code", "starter")
      .maybeSingle<{ id: string; currency: string; default_trial_days: number }>();
    if (starterPlan) {
      const now = new Date();
      const trialDays = Math.max(0, Number(starterPlan.default_trial_days ?? 14));
      ...
      await admin.from("organization_subscriptions").insert({
        ...
        currency: starterPlan.currency || "USD",
        ...
        metadata: { source: "demo_conversion", request_id: request.id },
      });
    }
  } catch (subscriptionError) {
    console.error("Starter trial assignment unavailable", subscriptionError);
  }
  ...
  await logAcquisition({
    ...
    details: { converted_organization_id: organization.id, owner_user_id: authUser.id },
  });

  revalidatePath("/dashboard/acquisition");
  go(`${organization.name} est maintenant une organisation cliente active.`, "success", "demo");
}
