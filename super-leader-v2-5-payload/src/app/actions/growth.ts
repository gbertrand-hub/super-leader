...
const developmentPrograms = new Set([
  "school_coaches",
  "school_business",
  "school_experts",
  "school_breeders",
...
async function getContext() {
  ...
  await enforceOrganizationFeature(membership.organization_id, "growth");
...
async function loadSettings(organizationId: string, admin: ReturnType<typeof createAdminClient>) {
  const {data} = await admin
    ...
    .select("default_monthly_target_hours,target_credits,bonus_weight,max_monthly_credits,night_start_time,night_end_time,wellbeing_warning_hours,default_development_target_hours,default_reading_target_hours,development_credit_per_hour,reading_credit_per_hour,max_development_credits")
...
export async function upsertGrowthPlanAction(formData: FormData) {
  ...
  const targetDevelopmentHours = parseNumber(formData.get("targetDevelopmentHours"), 12);
  const targetReadingHours = parseNumber(formData.get("targetReadingHours"), 2);
  ...
  if (!Number.isFinite(targetHours) || targetHours < 0 || targetHours > 200 || !Number.isFinite(targetCredits) || targetCredits < 1 || targetCredits > 500 || !Number.isFinite(targetDevelopmentHours) || targetDevelopmentHours < 0 || targetDevelopmentHours > 300 || !Number.isFinite(targetReadingHours) || targetReadingHours < 0 || targetReadingHours > 100 || focusSkill.length < 2 || objective.length < 5) {
  ...
  const planPayload = {
    ...
    target_development_hours: targetDevelopmentHours,
    target_reading_hours: targetReadingHours,
  ...
  await audit({organizationId: membership.organization_id, actorId: user.id, subjectUserId: targetUserId, action: "growth_plan_upserted", details: {month, targetHours, targetCredits, targetDevelopmentHours, targetReadingHours, focusSkill}});
...
export async function createDevelopmentActivityAction(formData: FormData) {
  const {user, membership, admin, t} = await getContext();
  const month = normalizeMonth(formData.get("month"));
  const activityDate = normalizeDate(formData.get("activityDate"));
  const startTime = normalizeTime(formData.get("startTime"));
  const endTime = normalizeTime(formData.get("endTime"));
  const crossesMidnight = formData.get("crossesMidnight") === "on";
  const programCode = clean(formData.get("programCode"), 60);
  const title = clean(formData.get("title"), 240);
  const author = clean(formData.get("author"), 180) || null;
  const learningSummary = clean(formData.get("learningSummary"), 2500);
  const applicationCommitment = clean(formData.get("applicationCommitment"), 2000);
  const evidenceUrlRaw = clean(formData.get("evidenceUrl"), 1000);
  const evidenceUrl = normalizeWebUrl(formData.get("evidenceUrl"));
  const timezone = normalizeTimeZone(clean(formData.get("timezone"), 120) || "Europe/Dublin");

  if (!activityDate || !startTime || !endTime || !developmentPrograms.has(programCode) || title.length < 2 || (evidenceUrlRaw && !evidenceUrl)) {
    go(t("growth.messages.invalidDevelopmentActivity"), "error", month);
  }
  if (programCode === "book_reading" && (learningSummary.length < 5 || applicationCommitment.length < 5)) {
    go(t("growth.messages.bookReflectionRequired"), "error", month);
  }

  const start = timeMinutes(startTime);
  const end = timeMinutes(endTime) + (crossesMidnight ? 1440 : 0);
  const durationMinutes = end - start;
  if (durationMinutes < 15 || durationMinutes > 1440) go(t("growth.messages.invalidDevelopmentDuration"), "error", month);

  const settings = await loadSettings(membership.organization_id, admin);
  if (!settings) go(t("growth.messages.migrationRequired"), "error", month);
  const nightStart = timeMinutes(settings.night_start_time);
  const nightEnd = timeMinutes(settings.night_end_time);
  const nightMinutes = calculateNightMinutes(start, end, nightStart, nightEnd);
  const weekendMinutes = calculateWeekendMinutes(activityDate, start, end);
  const id = randomUUID();

  const {error} = await admin.from("development_activities").insert({
    id,
    organization_id: membership.organization_id,
    user_id: user.id,
    ...
    application_commitment: applicationCommitment,
    evidence_url: evidenceUrl,
  });
  if (error) go(t("growth.messages.developmentCreateFailed", {message: error.message}), "error", month);

  const [{data: schedule}, {data: teams}] = await Promise.all([
    admin.from("member_work_schedules").select("supervisor_id").eq("organization_id", membership.organization_id).eq("user_id", user.id).eq("is_active", true).maybeSingle<{supervisor_id: string | null}>(),
    admin.from("team_members").select("teams!inner(manager_id)").eq("user_id", user.id).eq("teams.organization_id", membership.organization_id).eq("teams.is_active", true),
  ]);
  const teamManager = ((teams ?? []) as unknown as Array<{teams: {manager_id: string | null} | Array<{manager_id: string | null}>}>).map((row) => {
    const team = row.teams;
    return Array.isArray(team) ? team[0]?.manager_id : team?.manager_id;
  }).find(Boolean);
  const reviewerId = schedule?.supervisor_id || teamManager;
  if (reviewerId && reviewerId !== user.id) {
    await createNotification({
      ...
      actorId: user.id,
      category: "performance",
      eventType: "development_activity_submitted",
      titleFr: "Activité de développement à valider",
      titleEn: "Development activity to review",
      bodyFr: `${title} a été ajoutée au plan de croissance.`,
      bodyEn: `${title} was added to the growth plan.`,
      actionUrl: `/dashboard/growth?month=${activityDate.slice(0, 7)}&member=${user.id}`,
      priority: "info",
      requiresAction: true,
      dedupeKey: `development-review:${id}`,
      metadata: {development_activity_id: id, employee_id: user.id},
    });
  }

  await audit({organizationId: membership.organization_id, actorId: user.id, subjectUserId: user.id, entityId: id, action: "development_activity_submitted", details: {programCode, durationMinutes, nightMinutes, weekendMinutes}});
  revalidatePath("/dashboard/growth");
  go(t("growth.messages.developmentSubmitted"), "success", month || activityDate.slice(0, 7), user.id);
}

export async function cancelDevelopmentActivityAction(formData: FormData) {
  const {user, membership, admin, t} = await getContext();
  const activityId = clean(formData.get("activityId"), 80);
  const month = normalizeMonth(formData.get("month"));
  const {data: activity} = await admin.from("development_activities").select("id,user_id,status,source").eq("id", activityId).eq("organization_id", membership.organization_id).maybeSingle<{id: string; user_id: string; status: string; source: string}>();
  if (!activity || activity.user_id !== user.id || activity.source !== "manual" || activity.status !== "submitted") go(t("growth.messages.developmentCancelDenied"), "error", month);
  const {error} = await admin.from("development_activities").update({status: "cancelled"}).eq("id", activityId).eq("organization_id", membership.organization_id);
  if (error) go(t("growth.messages.developmentCancelFailed", {message: error.message}), "error", month);
  await audit({organizationId: membership.organization_id, actorId: user.id, subjectUserId: user.id, entityId: activityId, action: "development_activity_cancelled"});
  revalidatePath("/dashboard/growth");
  go(t("growth.messages.developmentCancelled"), "success", month, user.id);
}

export async function reviewDevelopmentActivityAction(formData: FormData) {
  const {user, membership, admin, visibleUserIds, t} = await getContext();
  const activityId = clean(formData.get("activityId"), 80);
  const month = normalizeMonth(formData.get("month"));
  const decision = clean(formData.get("decision"), 40);
  const approvedMinutes = Math.round(parseNumber(formData.get("approvedMinutes")));
  const reviewNote = clean(formData.get("reviewNote"), 2000);
  if (!reviewRoles.has(membership.role) || !["approved", "partially_approved", "rejected"].includes(decision) || reviewNote.length < 3) {
    go(t("growth.messages.invalidDevelopmentReview"), "error", month);
  }

  const {data: activity} = await admin.from("development_activities").select("id,user_id,title,program_code,duration_minutes,status,source").eq("id", activityId).eq("organization_id", membership.organization_id).maybeSingle<{id: string; user_id: string; title: string; program_code: string; duration_minutes: number; status: string; source: string}>();
  if (!activity || activity.source !== "manual" || !visibleUserIds.includes(activity.user_id) || activity.user_id === user.id || activity.status !== "submitted") go(t("growth.messages.developmentReviewDenied"), "error", month);

  const normalizedMinutes = decision === "rejected" ? 0 : approvedMinutes;
  if (!Number.isInteger(normalizedMinutes) || normalizedMinutes < 0 || normalizedMinutes > activity.duration_minutes || (decision === "approved" && normalizedMinutes !== activity.duration_minutes) || (decision === "partially_approved" && (normalizedMinutes <= 0 || normalizedMinutes >= activity.duration_minutes))) {
    go(t("growth.messages.invalidApprovedDuration"), "error", month, activity.user_id);
  }

  const settings = await loadSettings(membership.organization_id, admin);
  if (!settings) go(t("growth.messages.migrationRequired"), "error", month);
  const rate = activity.program_code === "book_reading" ? Number(settings.reading_credit_per_hour ?? 1.5) : Number(settings.development_credit_per_hour ?? 1);
  const growthCredits = decision === "rejected" ? 0 : Math.min(20, Math.round((normalizedMinutes / 60) * rate * 10) / 10);
  const {error} = await admin.from("development_activities").update({
    status: decision,
    approved_minutes: normalizedMinutes,
    growth_credits: growthCredits,
    review_note: reviewNote,
    reviewed_by: user.id,
    reviewed_at: new Date().toISOString(),
  }).eq("id", activityId).eq("organization_id", membership.organization_id);
  if (error) go(t("growth.messages.developmentReviewSaveFailed", {message: error.message}), "error", month, activity.user_id);
  ...
  await createNotification({
    ...
    actorId: user.id,
    category: "performance",
    eventType: "development_activity_reviewed",
    titleFr: "Activité de développement examinée",
    titleEn: "Development activity reviewed",
    bodyFr: `${activity.title} : ${decision === "rejected" ? "refusée" : `${growthCredits} crédit(s) de croissance validé(s)`}.`,
    bodyEn: `${activity.title}: ${decision === "rejected" ? "rejected" : `${growthCredits} growth credit(s) approved`}.`,
    actionUrl: `/dashboard/growth?month=${month || new Date().toISOString().slice(0, 7)}`,
    priority: decision === "rejected" ? "warning" : "success",
    requiresAction: false,
    dedupeKey: `development-reviewed:${activityId}:${decision}`,
    metadata: {development_activity_id: activityId, decision, growth_credits: growthCredits},
  });
  await audit({organizationId: membership.organization_id, actorId: user.id, subjectUserId: activity.user_id, entityId: activityId, action: `development_activity_${decision}`, details: {approvedMinutes: normalizedMinutes, growthCredits, reviewNote}});
  revalidatePath("/dashboard/growth");
  revalidatePath("/dashboard/performance");
  go(t("growth.messages.developmentReviewSaved"), "success", month, activity.user_id);
}
...
export async function updateGrowthSettingsAction(formData: FormData) {
  ...
  const defaultDevelopmentTargetHours = parseNumber(formData.get("defaultDevelopmentTargetHours"), 12);
  const defaultReadingTargetHours = parseNumber(formData.get("defaultReadingTargetHours"), 2);
  const developmentCreditPerHour = parseNumber(formData.get("developmentCreditPerHour"), 1);
  const readingCreditPerHour = parseNumber(formData.get("readingCreditPerHour"), 1.5);
  const maxDevelopmentCredits = parseNumber(formData.get("maxDevelopmentCredits"), 20);
  const nightStartTime = normalizeTime(formData.get("nightStartTime"));
  const nightEndTime = normalizeTime(formData.get("nightEndTime"));
  if (![defaultMonthlyTargetHours, targetCredits, bonusWeight, maxMonthlyCredits, wellbeingWarningHours, defaultDevelopmentTargetHours, defaultReadingTargetHours, developmentCreditPerHour, readingCreditPerHour, maxDevelopmentCredits].every(Number.isFinite) || defaultMonthlyTargetHours < 0 || defaultMonthlyTargetHours > 200 || targetCredits < 1 || targetCredits > 500 || bonusWeight < 0 || bonusWeight > 20 || maxMonthlyCredits < 1 || maxMonthlyCredits > 1000 || wellbeingWarningHours < 0 || wellbeingWarningHours > 200 || defaultDevelopmentTargetHours < 0 || defaultDevelopmentTargetHours > 300 || defaultReadingTargetHours < 0 || defaultReadingTargetHours > 100 || developmentCreditPerHour < 0 || developmentCreditPerHour > 20 || readingCreditPerHour < 0 || readingCreditPerHour > 20 || maxDevelopmentCredits < 0 || maxDevelopmentCredits > 500 || !nightStartTime || !nightEndTime) {
  ...
  const {error} = await admin.from("growth_settings").upsert({
    ...
    default_development_target_hours: defaultDevelopmentTargetHours,
    default_reading_target_hours: defaultReadingTargetHours,
    development_credit_per_hour: developmentCreditPerHour,
    reading_credit_per_hour: readingCreditPerHour,
    max_development_credits: maxDevelopmentCredits,
  ...
  await audit({organizationId: membership.organization_id, actorId: user.id, action: "growth_settings_updated", details: {defaultMonthlyTargetHours, targetCredits, bonusWeight, maxMonthlyCredits, wellbeingWarningHours, defaultDevelopmentTargetHours, defaultReadingTargetHours, developmentCreditPerHour, readingCreditPerHour, maxDevelopmentCredits, nightStartTime, nightEndTime}});
  revalidatePath("/dashboard/growth");
