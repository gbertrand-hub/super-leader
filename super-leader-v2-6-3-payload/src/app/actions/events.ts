...
function goEvent(text: string, kind: "success" | "error" = "success", eventId?: string): never {
...
export async function createEventAction(formData: FormData) {
  ...
  refreshEventViews();
  goEvent(copy.eventCreated, "success", event.id);
}
...
export async function updateEventAction(formData: FormData) {
  ...
  refreshEventViews();
  goEvent(copy.eventUpdated, "success", eventId);
}
...
export async function assignEventMemberAction(formData: FormData) {
  ...
  refreshEventViews();
  goEvent(copy.memberAssigned, "success", eventId);
}
...
export async function removeEventMemberAction(formData: FormData) {
  ...
  refreshEventViews();
  goEvent(copy.memberRemoved, "success", eventId);
}
...
export async function createEventTaskAction(formData: FormData) {
  ...
  refreshEventViews();
  goEvent(copy.taskCreated, "success", eventId);
}
...
export async function updateEventTaskAction(formData: FormData) {
  ...
  const eventId = first(formData.get("eventId"));
  const base = await context();
  const {data: task, error: taskError} = await base.admin.from("event_tasks").select("id,event_id,organization_id,assignee_id").eq("id", taskId).eq("organization_id", base.membership.organization_id).maybeSingle<TaskRow>();
  if (taskError || !task || task.event_id !== eventId) goEvent(base.copy.eventMissing, "error", eventId);
  const eventBase = await eventContext(eventId);
  const selfAssignee = task.assignee_id === base.user.id;
  if (!eventBase.canManage && !selfAssignee) goEvent(base.copy.permission, "error", eventId);

  const status = taskStatuses.has(first(formData.get("status"))) ? first(formData.get("status")) : "todo";
  ...
  if (eventBase.canManage && first(formData.get("actualCost"))) {
    updatePayload.actual_cost = Math.max(0, numberValue(formData.get("actualCost")));
  }
  const {error} = await base.admin.from("event_tasks").update(updatePayload).eq("id", taskId).eq("event_id", eventId);
  if (error) goEvent(message(base.copy.saveFailed, {message: error.message}), "error", eventId);
  await logEvent(base.admin, {organizationId: task.organization_id, eventId, actorId: base.user.id, action: "task_updated", targetUserId: task.assignee_id, details: {task_id: taskId, status, progress: status === "done" ? 100 : progress}});
  refreshEventViews();
  goEvent(base.copy.taskUpdated, "success", eventId);
}
...
export async function createEventScheduleItemAction(formData: FormData) {
  ...
  refreshEventViews();
  goEvent(copy.scheduleCreated, "success", eventId);
}
...
export async function addEventDocumentAction(formData: FormData) {
  ...
  refreshEventViews();
  goEvent(copy.documentAdded, "success", eventId);
}
...
export async function saveEventClosureReportAction(formData: FormData) {
  ...
  refreshEventViews();
  goEvent(copy.reportSaved, "success", eventId);
}
