...
const demoLabelsFr: Record<string, string> = {
  ...
  free_approved: "Plan Free approuvé",
...
const demoLabelsEn: Record<string, string> = {
  ...
  free_approved: "Free plan approved",
...
export default async function AcquisitionPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  if (canManageDemo) {
    let query = admin
      .from("demo_requests")
      .select("id,requester_user_id,full_name,email,phone,whatsapp,organization_name,country,sector,employee_count_range,requested_plan_code,needs,interested_modules,preferred_demo_date,status,assigned_to,scheduled_demo_at,sales_notes,converted_organization_id,created_at")
                ...
                <article key={request.id} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      ...
                      <p className="mt-2 inline-flex rounded-full bg-amber-50 px-3 py-1 text-xs font-black uppercase tracking-wide text-amber-900">
                        {fr ? "Plan demandé" : "Requested plan"}: {request.requested_plan_code || "starter"}
                      </p>
                  ...
                  {request.converted_organization_id ? (
                  ...
                  ) : ["demo_completed", "trial_approved", "free_approved"].includes(request.status) ? (
                    <form action={convertDemoRequestAction} className="mt-4">
                      ...
                      <button className="rounded-xl border border-emerald-300 bg-emerald-50 px-5 py-3 font-black text-emerald-800">
                        {request.requested_plan_code === "free"
                          ? fr
                            ? "Activer l’organisation Free"
                            : "Activate Free organization"
                          : fr
                            ? "Convertir en organisation cliente"
                            : "Convert to client organization"}
                      </button>
                    </form>
                  ) : (
                    <p className="mt-4 text-sm font-semibold text-slate-500">
                      {request.requested_plan_code === "free"
                        ? fr
                          ? "L’activation sera disponible après vérification et approbation du plan Free."
                          : "Activation becomes available after the Free plan request is reviewed and approved."
                        : fr
                          ? "La conversion sera disponible après la démonstration ou l’approbation de l’essai."
                          : "Conversion becomes available after the demo or trial approval."}
                    </p>
                  )}
                </article>
